...
class CameraAwarenessService extends ChangeNotifier {
  ...
  void _evaluate(DateTime now) {
    ...
    if (now.difference(since) < const Duration(milliseconds: 350)) return;
    _activate(camera, candidate.distanceMeters);
  }

  _Candidate? _selectCandidate() {
    final radius = _alertRadiusMeters(_vehicleSpeedKph);
    _Candidate? best;
    for (final camera in repository.records) {
      ...
      final aheadDelta = _angleDifference(_vehicleHeadingDeg, bearing);
      final aheadGate = _vehicleSpeedKph < 8 ? 88.0 : 62.0;
      if (aheadDelta > aheadGate) continue;

      final cameraHeading = camera.headingDeg;
      if (cameraHeading != null &&
          _angleDifference(_vehicleHeadingDeg, cameraHeading) > 48.0) {
        continue;
      }

      final score = distance + aheadDelta * 2.6 - camera.confidence * 85.0;
  ...
  void _updateActive(
    ...
    if (currentDisplay == null) {
      ...
      // A meaningful increase is allowed for a turn, detour or GNSS correction.
